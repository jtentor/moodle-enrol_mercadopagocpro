...
final class status
{
    ...
    public static function label(?string $status): string
    {
        ...
        if ($status === self::LOCAL_CREATED) {
            return get_string('status_created', 'enrol_mercadopagocpro');
        }
        if (self::is_known($status)) {
            return get_string('status_' . $status, 'enrol_mercadopagocpro');
        }
        return get_string('status_unknown', 'enrol_mercadopagocpro', s($status));
    }
}
